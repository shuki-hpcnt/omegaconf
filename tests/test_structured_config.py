...
class TestConfigs:
    ...
    def test_config_with_list(self, class_type):
        ...
        def validate(cfg):
            assert cfg == {"list1": [1, 2, 3], "list2": [1, 2, 3], "list3": [1, 2, 3]}
    ...
    def test_untyped(self, class_type, input_init, expected_init):
        ...
        assert type(conf.get_node("null_default")) == UntypedNode
        assert type(conf.get_node("int_default")) == UntypedNode
        assert type(conf.get_node("float_default")) == UntypedNode
        assert type(conf.get_node("str_default")) == UntypedNode
        assert type(conf.get_node("bool_default")) == UntypedNode
        assert type(conf.get_node("mandatory_missing")) == UntypedNode
    ...
    def test_typed_dict_value_error_structured(self, class_type):
        input_ = import_module(
            "tests.structured_conf." + class_type
        ).ErrorDictUnsupportedStructuredConfig
        with pytest.raises(ValidationError):
            OmegaConf.create(input_)

    def test_typed_list_value_error_structured(self, class_type):
        input_ = import_module(
            "tests.structured_conf." + class_type
        ).ErrorListUnsupportedStructuredConfig
        with pytest.raises(ValidationError):
            OmegaConf.create(input_)
